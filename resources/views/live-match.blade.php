    ...
    <main class="text-text-body pt-0 pb-32 sm:pt-2 sm:pb-32">
        <div class="max-w-4xl mx-auto px-4 pb-8"
             ...
             x-data="liveMatch({
                ...
                slotAssignments: {{ Js::from($slotAssignments) }},
                computeSlotsUrl: {{ Js::from($computeSlotsUrl) }},
                ...
                firstHalfStoppage: @js((int) ($match->first_half_stoppage ?? 0)),
                secondHalfStoppage: @js((int) ($match->second_half_stoppage ?? 3)),
                etFirstHalfStoppage: @js($match->et_first_half_stoppage),
                etSecondHalfStoppage: @js($match->et_second_half_stoppage),
                matchId: '{{ $match->id }}',
                animationSeen: {{ $animationSeen ? 'true' : 'false' }},
                homeForm: @js($homeForm),
                awayForm: @js($awayForm),
                homePosition: @js($homePosition),
                ...
                translations: {
                    ...
                    etHalfTime: {!! Js::from(__('game.live_et_half_time')) !!},
                    penalties: {!! Js::from(__('game.live_penalties')) !!},
                    ...
                    tournamentFourth: {!! Js::from(__('game.tournament_fourth_place_title')) !!},
                    tournamentEliminated: {!! Js::from(__('game.tournament_eliminated_title')) !!},
                    tournamentEliminatedIn: {!! Js::from(__('game.tournament_eliminated_in', ['round' => $knockoutRoundName ?? ''])) !!},
                    tournamentAdvance: {!! Js::from(__('game.tournament_you_advance')) !!},
                    tournamentAdvanceTo: {!! Js::from(__('game.tournament_advance_to', ['round' => ''])) !!},
                    tournamentToFinal: {!! Js::from(__('game.tournament_to_final')) !!},
                    tournamentToThirdPlace: {!! Js::from(__('game.tournament_to_third_place')) !!},
                    tournamentViewSummary: {!! Js::from(__('game.tournament_view_summary')) !!},
        ...
        >
            ...
            <div>
                ...
                <div class="px-4 sm:px-6 md:px-8 py-3 sm:py-4">
                    ...
                    <div class="flex items-center justify-center gap-1.5 pb-3" x-show="phase !== 'full_time' && phase !== 'half_time' && phase !== 'going_to_extra_time' && phase !== 'extra_time_half_time' && !penaltyPickerOpen">
                        {{-- Pause/Play --}}
                        <x-secondary-button
                            size="sm"
                            @click="togglePause()"
                            class="!px-2.5"
                            x-bind:class="userPaused ? '!text-accent-gold' : ''">
                        ...
                        {{-- Speed selector --}}
                        <div class="inline-flex bg-surface-700 rounded-lg p-0.5 border border-border-strong min-h-[36px] items-center">
                            <template x-for="s in [1, 2, 4]" :key="s">
                                <button
                                    @click="setSpeed(s)"
                                    class="text-xs font-semibold tracking-wide w-9 h-8 rounded-md transition-colors flex items-center justify-center"
                                    :class="speed === s
                        ...
                        <x-secondary-button
                            ...
                            class="!px-2.5"
                            x-bind:title="translations.skipToHt">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-4">
                                <path d="M3.288 4.818A1.5 1.5 0 0 0 1 6.095v7.81a1.5 1.5 0 0 0 2.288 1.276l6.323-3.905c.155-.096.285-.213.389-.344v2.973a1.5 1.5 0 0 0 2.288 1.276l6.323-3.905a1.5 1.5 0 0 0 0-2.552l-6.323-3.906A1.5 1.5 0 0 0 10 6.095v2.973a1.506 1.506 0 0 0-.389-.344L3.288 4.818Z" />
                                <path d="M19 4.75a.75.75 0 0 0-.75-.75h-.5a.75.75 0 0 0-.75.75v10.5c0 .414.336.75.75.75h.5a.75.75 0 0 0 .75-.75V4.75Z" />
                            </svg>
                        </x-secondary-button>
                        ...
                        <x-secondary-button
                            ...
                            x-bind:title="translations.skipToFt">
                    ...
                    <div class="flex items-center justify-center gap-1.5 pb-3" x-show="phase === 'half_time'" x-cloak>
                        <x-secondary-button
                            size="sm"
                    ...
                    <div class="flex items-center justify-center gap-1.5 pb-3" x-show="phase === 'going_to_extra_time'" x-cloak>
                        <x-secondary-button
                            size="sm"
                            @click="openTacticalPanel('substitutions')"
                            x-bind:disabled="extraTimeLoading"
                    ...
                    <div class="flex items-center justify-center gap-1.5 pb-3" x-show="phase === 'extra_time_half_time'" x-cloak>
                        <x-secondary-button
                            size="sm"
                            @click="openTacticalPanel('substitutions')"
                            class="gap-1.5">
